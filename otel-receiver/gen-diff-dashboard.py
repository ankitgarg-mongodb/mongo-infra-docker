...
emitted four times - one column per backend (prometheus, grafana-otel,
victoriametrics, greptimedb) - at identical grid positions, so rows align for
scroll comparison. The six source stat panels are folded into one combined stat
panel per backend (side by side at w8 they would not fit the 24-column grid
otherwise).
...
SRC_SUFFIX = {"prom": "  prom ▶", "graf": "  graf ▶", "vm": "  vm ▶", "gt": "  gt ▶"}
DS = {
    "prom": {"type": "prometheus", "uid": "prometheus"},
    "graf": {"type": "prometheus", "uid": "grafana-otel"},
    "vm": {"type": "prometheus", "uid": "victoriametrics"},
    "gt": {"type": "prometheus", "uid": "greptimedb-prom"},
}
COLS = (("prom", 0), ("graf", 6), ("vm", 12), ("gt", 18))
...
diff = {
    ...
    "description": (
        "Four-way drift comparison: prometheus | grafana-otel | victoriametrics | greptimedb - the same panels fed "
        "by the agent's export paths. The same metric on every column should show the same shape; divergence = a "
        "backend dropped or lagged data. Fully label-driven like the source dashboard."
    ),
...
for i, (side, x) in enumerate(COLS):
    targets = []
